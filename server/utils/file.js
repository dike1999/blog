  ...
  if (hasPrefix) {
    ...
    const _decodePrefix = (prefixStr) => {
      ...
      const _loop = (prev, next) => {
        ...
        const trimStr = end === -1
          ? prefixData.slice(start).trim()
          : prefixData.slice(start, end).trim(); // 字符串截取 + trim
...
function createFileContent({
  ...
  function _generateTag(list) {
    const newList = list.reduce((list2, item) => {
...
async function generateFile(article) {
  return new Promise((resolve, reject) => {
    findOrCreateFilePath(outputPath).then(() => {
      ...
      fs.writeFile(writeFilePath, fileContent, (err) => {
        if (err) {
          reject();
          throw err;
        }
        fs.readFile(writeFilePath, (err2) => {
          if (err2) {
            throw err2;
          }
