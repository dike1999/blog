...
function getEmailList(target, userId) {
  ...
  loop(target);
  if (target.replies) {
    target.replies.forEach((r) => loop(r));
  }
...
exports.getEmailData = (article, disscussData, userId) => {
  ...
  const link = article.id !== -1
    ? `${WEB_HOST}/article/${article.id}#discuss`
    : `${WEB_HOST}/about`;
  ...
  const HTML_CONTENT = `
    ...
    <div style="padding-left: 15px;">${disscussData.replies
    .map(createDisscusInfo)
    .join('')}</div>
  `;
